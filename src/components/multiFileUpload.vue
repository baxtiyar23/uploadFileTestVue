...
const multipleUpload = async () => {
  ...
  for (let i = 0; i < multiResData.value.length; i++) {
    try {
      if (multiResData.value[i].status !== statusIcon[STATUS.success]) {
        if (multiResData.value[i].size < props.maxSize) {
          ...
          if (response.status == 200) {
            multiResData.value[i].status = statusIcon[STATUS.success]
          }
        } else {
          multiResData.value[i].status = statusIcon[STATUS.failed]
        }
      }
      multiLoader.value = false
    } catch (error) {
      multiResData.value[i].status = statusIcon[STATUS.error]
      multiLoader.value = false
    }
  }
}
    ...
    <div class="flex flex-wrap gap-2">
      <div class="flex flex-wrap" v-for="item in multiResData" key="item.id">
        <div class="relative">
          ...
          <img class="absolute top-1 left-1 w-[30px]" :src="item.status" alt="" />
